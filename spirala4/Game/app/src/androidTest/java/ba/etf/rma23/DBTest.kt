...
import ba.etf.rma23.data.repositories.GameReviewsRepository
...
class DBTest {
    ...
    private val HASH = AccountGamesRepository.getHash()
    private val idIGRE = 18


    private val countNotOnline =
        "SELECT COUNT(*) AS broj_reviews FROM gamereview WHERE online=0" //jer ne prepoznaje false
    ...
    fun a3_sendWhileOffline() = runBlocking {
        ...
        Thread.sleep(5000) //promijenjeno zbog a4 testa
        var rez =
    ...
    fun a4_enableInternetAndSendOffline() = runBlocking {
        ...
        Thread.sleep(10000) //treba vremena da se poveze

        var rez = GameReviewsRepository.sendOfflineReviews(context)
        assertEquals(rez, 1)

        Log.d("response:", "${GameReviewsRepository.getOfflineReviews(context)}")
        executeCountAndCheck(countNotOnline, "broj_reviews", 0)
